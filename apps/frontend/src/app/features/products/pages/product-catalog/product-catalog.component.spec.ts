...
describe('ProductCatalogComponent', () => {
    ...
    beforeEach(async () => {
        const productSpy = jasmine.createSpyObj('ProductService', ['getProducts', 'getCategories']) as jasmine.SpyObj<ProductService>;
        productSpy.getProducts.and.returnValue(
            of({
                data: [
                    { id: 'p1', name: 'Laptop', category: 'Electronics' },
                    { id: 'p2', name: 'Book', category: 'Books' },
                ],
                total: 2,
                page: 1,
                limit: 12,
    ...
    it('onProductSelect navigates to detail', () => {
        component.onProductSelect({ id: 'p1' } as any);
