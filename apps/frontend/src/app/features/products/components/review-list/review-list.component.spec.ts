...
describe('ReviewListComponent', () => {
    ...
    beforeEach(async () => {
        const authSpy = jasmine.createSpyObj('AuthService', ['getCurrentUser']) as jasmine.SpyObj<AuthService>;
        authSpy.getCurrentUser.and.returnValue(of({ id: 'u1' } as any));
        const reviewSpy = jasmine.createSpyObj('ReviewService', ['deleteReview']) as jasmine.SpyObj<ReviewService>;
        ...
        component.reviews = [{ id: 'r1', userId: 'u1', rating: 5, comment: 'ok' } as any];
    ...
    it('isOwner returns true for current user', () => {
        expect(component.isOwner(component.reviews[0] as any)).toBeTrue();
    });
