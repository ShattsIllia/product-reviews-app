...
import type { CreateReviewInput, UpdateReviewInput, PaginatedResponseDto, ReviewDto } from '../../../core/models/api';
...
export class ReviewService {
    ...
    getReviewsByProduct(
        ...
        return this.http
            .get<PaginatedResponseDto<ReviewDto>>(`${this.configService.getProductsUrl()}/${productId}/reviews`, {
                params: { page, limit },
            })
    ...
    updateReview(reviewId: string, data: UpdateReviewInput): Observable<ReviewModel> {
        return this.http.patch<ReviewDto>(`${this.configService.getReviewsUrl()}/${reviewId}`, data).pipe(map(mapReviewDto));
    }

    deleteReview(reviewId: string): Observable<{ statusCode: number }> {
        return this.http.delete<{ statusCode: number }>(`${this.configService.getReviewsUrl()}/${reviewId}`);
    }
}
